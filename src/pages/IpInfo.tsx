...
import { getMyIp } from '@/services/api';
...
useEffect(() => {
  ...
  const handleQuery = async (useCommercial: boolean) => {
    ...
    try {
      const data = await getMyIp(useCommercial);
      setIpData(data);
    } catch (err: any) {
    ...
    <div className="min-h-[calc(100vh-400px)] py-12" style={{ backgroundColor: '#f5f5f7' }}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={headerRef} className="text-center mb-12">
          ...
          >
            <HiGlobeAlt className="text-4xl" style={{ color: '#0071e3' }} />
          </div>
          <h1 className="text-3xl font-semibold text-apple-text mb-3">
            公网 IP 查询
          </h1>
          <p className="text-gray-600 max-w-2xl mx-auto">
            查询您的公网IP地址及详细归属信息，包括地理位置、运营商等数据
          </p>
        </div>
          ...
          <div className="space-y-4">
            ...
            >
              <div className="flex gap-3">
                <HiViewfinderCircle className="text-lg flex-shrink-0 mt-0.5" style={{ color: '#1e40af' }} />
                <div className="text-xs" style={{ color: '#1e40af' }}>
                  <p className="font-medium mb-1">温馨提示</p>
                  <p>商业查询提供更精确的市、区、运营商、时区、海拔等信息，但响应时间可能稍长。</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {loading && (
          <div 
            ...
            style={{ backgroundColor: '#ffffff' }}
          >
            <LoadingSpinner message="正在查询您的IP信息..." />
          </div>
        )}
        ...
        {error && (
          ...
          >
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold mb-2" style={{ color: '#dc2626' }}>查询失败</h3>
